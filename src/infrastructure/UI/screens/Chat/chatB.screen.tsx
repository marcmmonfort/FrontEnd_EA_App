...
//type ChatRouteProp = RouteProp<{ ChatB: { roomID: string } }, 'ChatB'>;
...
//export default function ChatB() {
  ...
  useEffect(() => {
    ...
    try{
        socketRef.current=io('http://147.83.7.158:9000');// Address of the Signal server REVISAR
    }catch(error){
  ...
  function handleNegotiationNeededEvent(userID: string) {
    ...
    peerRef.current.createOffer()
      ...
      .catch((err: Error) => console.log("Error handling negotiation needed event", err));
  }
  ...
  function handleReceiveMessage(e: { data: any; }){//revisar
    ...
    setMessages(previousMessages => GiftedChat.append(previousMessages, msg) as unknown as never)//Revisar
  };
...
function sendMessage(messages = []) {
    if (messages.length > 0) {
      ...
      setMessages(previousMessages => GiftedChat.append(previousMessages, messages) as unknown as never);
    }
  }
